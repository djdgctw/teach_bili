...
//------------------------Parameter----------------------
`define TV_IN_input_r "../tv/cdatafile/c.sobel_edge.autotvin_input_r.dat"
`define TV_IN_output_r "../tv/cdatafile/c.sobel_edge.autotvin_output_r.dat"
`define TV_IN_rows "../tv/cdatafile/c.sobel_edge.autotvin_rows.dat"
`define TV_IN_cols "../tv/cdatafile/c.sobel_edge.autotvin_cols.dat"
...
parameter sobel_edge_continue_addr = 0;
parameter sobel_edge_auto_start_addr = 0;
parameter input_r_data_in_addr = 16;
parameter output_r_data_in_addr = 28;
parameter rows_data_in_addr = 40;
parameter cols_data_in_addr = 48;
...
assign TRAN_control_write_data_finish = 1 & input_r_write_data_finish & output_r_write_data_finish & rows_write_data_finish & cols_write_data_finish;
        ...
        if (ongoing_process_number === process_num && process_busy === 0 ) begin
            ...
            if (write_rows_run_flag === 1 && get_ack === 1) begin
                ...
                for (i = 0 ; i < four_byte_num ; i = i+1) begin
                    ...
                    if(image_mem_rows[write_rows_count * four_byte_num  + i]!==rows_data_tmp_reg) begin
                    write (rows_data_in_addr + write_rows_count * four_byte_num * 4 + i * 4, rows_data_tmp_reg, write_rows_resp);
                    image_mem_rows[write_rows_count * four_byte_num + i]=rows_data_tmp_reg;
                    end
                end
                process_busy = 0;
        ...
        if (ongoing_process_number === process_num && process_busy === 0 ) begin
            ...
            if (write_cols_run_flag === 1 && get_ack === 1) begin
                ...
                for (i = 0 ; i < four_byte_num ; i = i+1) begin
                    ...
                    if(image_mem_cols[write_cols_count * four_byte_num  + i]!==cols_data_tmp_reg) begin
                    write (cols_data_in_addr + write_cols_count * four_byte_num * 4 + i * 4, cols_data_tmp_reg, write_cols_resp);
                    image_mem_cols[write_cols_count * four_byte_num + i]=cols_data_tmp_reg;
                    end
                end
                process_busy = 0;
