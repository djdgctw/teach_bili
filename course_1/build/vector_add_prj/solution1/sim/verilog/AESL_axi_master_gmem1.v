...
//------------------------Parameter----------------------
`define TV_IN_gmem1 "../tv/cdatafile/c.vector_add.autotvin_gmem1.dat"
`define TV_OUT_gmem1 "../tv/rtldatafile/rtl.vector_add.autotvout_gmem1.dat"
...
parameter gmem1_mem_depth = 32'd 256;
    ...
    while (1) begin
        if (FIFO_AW_req_empty === 1 || BRESP_counter == 32) begin
            ...
            if (FIFO_AW_req_ADDR_tmp/data_byte_size > gmem1_mem_depth) begin
                $display ("/home/fyt/A/teach_bili/course_1/build/vector_add_prj/solution1/sim/verilog/AESL_axi_master_gmem1.v: Write request address %d exceed AXI master gmem1 array depth: %d",FIFO_AW_req_ADDR_tmp/data_byte_size, gmem1_mem_depth); 
                $finish;
            end
    ...
    while (1) begin
        ...
        if (FIFO_AR_req_empty) begin
            ...
            if (FIFO_AR_req_ADDR_tmp/data_byte_size > gmem1_mem_depth) begin
                $display ("/home/fyt/A/teach_bili/course_1/build/vector_add_prj/solution1/sim/verilog/AESL_axi_master_gmem1.v: Read request address %d exceed AXI master gmem1 array depth: %d",FIFO_AR_req_ADDR_tmp/data_byte_size, gmem1_mem_depth); 
                $finish;
            end
  ...
  while(1) begin 
      ...
      for(i = 0; i < (gmem1_mem_depth - gmem1_mem_depth % factor); i = i + 1) begin 
          if (factor == 4) begin
              case(mem_page)
                  ...
                  3 : tmp_data = gmem1_mem_3[i/factor];
      ...
      if (factor == 4) begin
          case(mem_page)
              ...
              3 : tmp_data = gmem1_mem_3[gmem1_mem_depth/factor];
              default: $display("The page_num of write file is not valid!");
          endcase
      ...
      if (factor == 2) begin
          if ((gmem1_mem_depth - 1) % factor == 0) begin
              case(mem_page)
                  ...
                  3 : tmp_data = gmem1_mem_3[gmem1_mem_depth/factor][15:0];
                  default: $display("The page_num of write file is not valid!");
              endcase
              bin_data = tmp_data;
              write_binary(fp,bin_data,16);
